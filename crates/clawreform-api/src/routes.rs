//! Route handlers for the ClawReform API.
...
pub struct AppState {
    ...
    pub peer_registry: Option<Arc<clawreform_wire::registry::PeerRegistry>>,
    /// Channel bridge manager — held behind a Mutex so it can be swapped on hot-reload.
    pub bridge_manager: tokio::sync::Mutex<Option<clawreform_channels::bridge::BridgeManager>>,
    /// Live channel config — updated on every hot-reload so list_channels() reflects reality.
    pub channels_config: tokio::sync::RwLock<clawreform_types::config::ChannelsConfig>,
        ...
        match state.kernel.verify_signed_manifest(signed_json) {
            ...
            Err(e) => {
                ...
                state.kernel.audit_log.record(
                    "system",
                    clawreform_runtime::audit::AuditAction::AuthAttempt,
...
) -> Vec<clawreform_types::message::ContentBlock> {
    use base64::Engine;

    let upload_dir = std::env::temp_dir().join("clawreform_uploads");
    ...
    for att in attachments {
        ...
        match std::fs::read(&file_path) {
            Ok(data) => {
                ...
                blocks.push(clawreform_types::message::ContentBlock::Image {
...
pub fn inject_attachments_into_session(
    ...
    agent_id: AgentId,
    image_blocks: Vec<clawreform_types::message::ContentBlock>,
) {
    use clawreform_types::message::{Message, MessageContent, Role};
    ...
    let mut session = match kernel.memory.get_session(entry.session_id) {
        Ok(Some(s)) => s,
        _ => clawreform_memory::session::Session {
            id: entry.session_id,
            agent_id,
            messages: Vec::new(),
            ...
            let messages: Vec<serde_json::Value> = session
                ...
                .filter_map(|m| {
                    ...
                    let content = match &m.content {
                        clawreform_types::message::MessageContent::Text(t) => t.clone(),
                        clawreform_types::message::MessageContent::Blocks(blocks) => {
                            // Extract human-readable text and tool info from blocks
                            let mut texts = Vec::new();
                            for b in blocks {
                                match b {
                                    clawreform_types::message::ContentBlock::Text { text } => {
                                        texts.push(text.clone());
                                    }
                                    clawreform_types::message::ContentBlock::Image { .. } => {
                                    ...
                                    clawreform_types::message::ContentBlock::ToolResult {
                                        content: result,
                                        is_error,
                                        ..
                                    } => {
...
pub async fn shutdown(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    state.kernel.audit_log.record(
        "system",
        clawreform_runtime::audit::AuditAction::ConfigChange,
...
const CHANNEL_REGISTRY: &[ChannelMeta] = &[
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "user_id", label: "Bot User ID", field_type: FieldType::Text, env_var: None, required: false, placeholder: "@clawreform:matrix.org", advanced: true },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawreform_bot", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Bot Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawreform_bot", advanced: false },
            ...
            ChannelField { key: "subreddits", label: "Subreddits", field_type: FieldType::List, env_var: None, required: false, placeholder: "clawreform, rust", advanced: true },
            ChannelField { key: "default_agent", label: "Default Agent", field_type: FieldType::Text, env_var: None, required: false, placeholder: "assistant", advanced: true },
        ],
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawreform", advanced: false },
            ChannelField { key: "channels", label: "Channels", field_type: FieldType::List, env_var: None, required: false, placeholder: "#clawreform, #general", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Bot Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawreform", advanced: false },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "topic", label: "Topic", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawreform-alerts", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "clawreform", advanced: false },
...
fn is_channel_configured(config: &clawreform_types::config::ChannelsConfig, name: &str) -> bool {
    match name {
...
pub async fn configure_channel(
    ...
    let home = clawreform_kernel::config::clawreform_home();
...
pub async fn whatsapp_qr_start() -> impl IntoResponse {
    ...
    if gateway_url.is_empty() {
        return Json(serde_json::json!({
            ...
            "help": "Run: npx clawreform-whatsapp-gateway   (or set WHATSAPP_WEB_GATEWAY_URL)"
        }));
    }
...
pub async fn list_templates() -> impl IntoResponse {
    let agents_dir = clawreform_kernel::config::clawreform_home().join("agents");
...
///
/// Returns counters and gauges for monitoring ClawReform in production:
/// - `clawreform_agents_active` — number of active agents
/// - `clawreform_uptime_seconds` — seconds since daemon started
/// - `clawreform_tokens_total` — total tokens consumed (per agent)
/// - `clawreform_tool_calls_total` — total tool calls (per agent)
/// - `clawreform_panics_total` — supervisor panic count
/// - `clawreform_restarts_total` — supervisor restart count
pub async fn prometheus_metrics(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let mut out = String::with_capacity(2048);

    // Uptime
    let uptime = state.started_at.elapsed().as_secs();
    out.push_str("# HELP clawreform_uptime_seconds Time since daemon started.\n");
    out.push_str("# TYPE clawreform_uptime_seconds gauge\n");
    out.push_str(&format!("clawreform_uptime_seconds {uptime}\n\n"));
    ...
    let active = agents
        ...
        .count();
    out.push_str("# HELP clawreform_agents_active Number of active agents.\n");
    out.push_str("# TYPE clawreform_agents_active gauge\n");
    out.push_str(&format!("clawreform_agents_active {active}\n"));
    out.push_str("# HELP clawreform_agents_total Total number of registered agents.\n");
    out.push_str("# TYPE clawreform_agents_total gauge\n");
    out.push_str(&format!("clawreform_agents_total {}\n\n", agents.len()));

    // Per-agent token and tool usage
    out.push_str("# HELP clawreform_tokens_total Total tokens consumed (rolling hourly window).\n");
    out.push_str("# TYPE clawreform_tokens_total gauge\n");
    out.push_str("# HELP clawreform_tool_calls_total Total tool calls (rolling hourly window).\n");
    out.push_str("# TYPE clawreform_tool_calls_total gauge\n");
    for agent in &agents {
        let name = &agent.name;
        ...
        if let Some((tokens, tools)) = state.kernel.scheduler.get_usage(agent.id) {
            out.push_str(&format!(
                "clawreform_tokens_total{{agent=\"{name}\",provider=\"{provider}\",model=\"{model}\"}} {tokens}\n"
            ));
            out.push_str(&format!(
                "clawreform_tool_calls_total{{agent=\"{name}\"}} {tools}\n"
            ));
        }
    }
    out.push('\n');
    ...
    // Supervisor health
    let health = state.kernel.supervisor.health();
    out.push_str("# HELP clawreform_panics_total Total supervisor panics since start.\n");
    out.push_str("# TYPE clawreform_panics_total counter\n");
    out.push_str(&format!("clawreform_panics_total {}\n", health.panic_count));
    out.push_str("# HELP clawreform_restarts_total Total supervisor restarts since start.\n");
    out.push_str("# TYPE clawreform_restarts_total counter\n");
    out.push_str(&format!(
        "clawreform_restarts_total {}\n\n",
        health.restart_count
    ));

    // Version info
    out.push_str("# HELP clawreform_info ClawReform version and build info.\n");
    out.push_str("# TYPE clawreform_info gauge\n");
    out.push_str(&format!(
        "clawreform_info{{version=\"{}\"}} 1\n",
...
pub async fn install_skill(
    ...
    let skills_dir = state.kernel.config.home_dir.join("skills");
    let config = clawreform_skills::marketplace::MarketplaceConfig::default();
    let client = clawreform_skills::marketplace::MarketplaceClient::new(config);
...
pub async fn clawhub_search(
    ...
    let client = clawreform_skills::clawhub::ClawHubClient::new(cache_dir);
...
pub async fn clawhub_browse(
    ...
    let sort = match params.get("sort").map(|s| s.as_str()) {
        Some("downloads") => clawreform_skills::clawhub::ClawHubSort::Downloads,
        Some("stars") => clawreform_skills::clawhub::ClawHubSort::Stars,
        Some("updated") => clawreform_skills::clawhub::ClawHubSort::Updated,
        Some("rating") => clawreform_skills::clawhub::ClawHubSort::Rating,
        _ => clawreform_skills::clawhub::ClawHubSort::Trending,
    };
...
fn clawhub_browse_entry_to_json(
    entry: &clawreform_skills::clawhub::ClawHubBrowseEntry,
) -> serde_json::Value {
    let version = clawreform_skills::clawhub::ClawHubClient::entry_version(entry);
    serde_json::json!({
        "slug": entry.slug,
...
pub async fn activate_hand(
    State(state): State<Arc<AppState>>,
    Path(hand_id): Path<String>,
    body: Option<Json<clawreform_hands::ActivateHandRequest>>,
) -> impl IntoResponse {
...
pub async fn hand_instance_browser(
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            clawreform_runtime::browser::BrowserCommand::ReadPage,
        )
        .await
    {
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            clawreform_runtime::browser::BrowserCommand::Screenshot,
        )
        .await
    {
        Ok(resp) if resp.success => {
...
pub async fn update_budget(
    ...
    let config_ptr = &state.kernel.config as *const clawreform_types::config::KernelConfig
        as *mut clawreform_types::config::KernelConfig;
...
pub async fn agent_budget_status(
    ...
    let usage_store = clawreform_memory::usage::UsageStore::new(state.kernel.memory.usage_conn());
...
pub async fn delete_session(
    ...
    let session_id = match id.parse::<uuid::Uuid>() {
        Ok(u) => clawreform_types::agent::SessionId(u),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn set_session_label(
    ...
    if let Some(lbl) = label {
        if let Err(e) = clawreform_types::agent::SessionLabel::new(lbl) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": e.to_string()})),
            );
...
pub async fn find_session_by_label(
    ...
    let agent_id = match agent_id_str.parse::<uuid::Uuid>() {
        Ok(u) => clawreform_types::agent::AgentId(u),
        Err(_) => {
            // Try name lookup
...
pub async fn migrate_detect() -> impl IntoResponse {
    match clawreform_migrate::openclaw::detect_openclaw_home() {
        Some(path) => {
            let scan = clawreform_migrate::openclaw::scan_openclaw_workspace(&path);
            (
                StatusCode::OK,
                Json(serde_json::json!({
                    "detected": true,
...
pub async fn run_migrate(Json(req): Json<MigrateRequest>) -> impl IntoResponse {
    let source = match req.source.as_str() {
        "openclaw" => clawreform_migrate::MigrateSource::OpenClaw,
        "langchain" => clawreform_migrate::MigrateSource::LangChain,
        "autogpt" => clawreform_migrate::MigrateSource::AutoGpt,
        other => {
            return (
                StatusCode::BAD_REQUEST,
                Json(
    ...
    let models: Vec<serde_json::Value> = catalog
        ...
        .filter(|m| {
            ...
            if available_only {
                ...
                if let Some(p) = provider {
                    if p.auth_status == clawreform_types::model_catalog::AuthStatus::Missing {
                        return false;
                    }
                }
            }
        ...
        .map(|m| {
            let available = catalog
                ...
                .map(|p| p.auth_status != clawreform_types::model_catalog::AuthStatus::Missing)
                .unwrap_or(false);
            serde_json::json!({
                "id": m.id,
...
pub async fn list_providers(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let provider_list: Vec<clawreform_types::model_catalog::ProviderInfo> = {
        let catalog = state
            .kernel
            .model_catalog
            .read()
...
pub async fn a2a_agent_card(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if let Some(first) = agents.first() {
        let card = clawreform_runtime::a2a::build_agent_card(&first.manifest, &base_url);
        (
            StatusCode::OK,
...
pub async fn a2a_list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let cards: Vec<serde_json::Value> = agents
        .iter()
        .map(|entry| {
            let card = clawreform_runtime::a2a::build_agent_card(&entry.manifest, &base_url);
...
pub async fn a2a_send_task(
    ...
    let task = clawreform_runtime::a2a::A2aTask {
        id: task_id.clone(),
        session_id: session_id.clone(),
        status: clawreform_runtime::a2a::A2aTaskStatus::Working,
        messages: vec![clawreform_runtime::a2a::A2aMessage {
            role: "user".to_string(),
            parts: vec![clawreform_runtime::a2a::A2aPart::Text {
    ...
    match state.kernel.send_message(agent.id, &message_text).await {
        Ok(result) => {
            let response_msg = clawreform_runtime::a2a::A2aMessage {
        ...
        Err(e) => {
            let error_msg = clawreform_runtime::a2a::A2aMessage {
                role: "agent".to_string(),
                parts: vec![clawreform_runtime::a2a::A2aPart::Text {
                    text: format!("Error: {e}"),
                }],
...
pub async fn a2a_discover_external(
    ...
    let client = clawreform_runtime::a2a::A2aClient::new();
...
pub async fn mcp_http(
    ...
    let mut tools = builtin_tool_definitions();
    {
        ...
        for skill_tool in registry.all_tool_definitions() {
            tools.push(clawreform_types::tool::ToolDefinition {
    ...
    if method == "tools/call" {
        ...
        let kernel_handle: Arc<dyn clawreform_runtime::kernel_handle::KernelHandle> =
            state.kernel.clone() as Arc<dyn clawreform_runtime::kernel_handle::KernelHandle>;
        let result = clawreform_runtime::tool_runner::execute_tool(
            "mcp-http",
            tool_name,
            &arguments,
            Some(&kernel_handle),
    ...
    let response = clawreform_runtime::mcp_server::handle_mcp_request(&request, &tools).await;
    Json(response)
}
...
pub async fn switch_agent_session(
    ...
    let session_id = match session_id_str.parse::<uuid::Uuid>() {
        Ok(uuid) => clawreform_types::agent::SessionId(uuid),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid session ID"})),
...
pub async fn get_agent_mcp_servers(
    ...
    if let Ok(mcp_tools) = state.kernel.mcp_tools.lock() {
        ...
        for tool in mcp_tools.iter() {
            if let Some(server) = clawreform_runtime::mcp::extract_mcp_server(&tool.name) {
...
///
/// SECURITY: Writes to `~/.clawreform/secrets.env`, sets env var in process,
...
pub async fn test_provider(
    ...
    let driver_config = clawreform_runtime::llm_driver::DriverConfig {
        provider: name.clone(),
        api_key,
    ...
    match clawreform_runtime::drivers::create_driver(&driver_config) {
        Ok(driver) => {
            // Send a minimal completion request to test connectivity
            let test_req = clawreform_runtime::llm_driver::CompletionRequest {
                model: String::new(), // Driver will use default
                messages: vec![clawreform_types::message::Message::user("Hi")],
                tools: vec![],
                max_tokens: 1,
                temperature: 0.0,
                system: None,
...
pub async fn create_skill(
    ...
    // Write skill.toml to ~/.clawreform/skills/{name}/
    ...
    for info in registry.list_all_info() {
        ...
        let status = match &info.installed {
            ...
            Some(_) => match h.as_ref().map(|h| &h.status) {
                Some(clawreform_extensions::IntegrationStatus::Ready) => "ready",
                Some(clawreform_extensions::IntegrationStatus::Error(_)) => "error",
                _ => "installed",
            },
...
pub async fn add_integration(
    ...
    let install_err = {
        ...
        if registry.is_installed(&id) {
            ...
            let entry = clawreform_extensions::InstalledIntegration {
                id: id.clone(),
...
/// Must match the value in `clawreform-kernel/src/kernel.rs::shared_memory_agent_id()`.
...
const SCHEDULES_KEY: &str = "__clawreform_schedules";
...
pub async fn upload_file(
    ...
    let transcription = if content_type.starts_with("audio/") {
        let attachment = clawreform_types::media::MediaAttachment {
            media_type: clawreform_types::media::MediaType::Audio,
            mime_type: content_type.clone(),
            source: clawreform_types::media::MediaSource::FilePath {
...
pub async fn delete_cron_job(
    ...
    match uuid::Uuid::parse_str(&id) {
        Ok(uuid) => {
            let job_id = clawreform_types::scheduler::CronJobId(uuid);
...
pub async fn webhook_wake(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<clawreform_types::webhook::WakePayload>,
) -> impl IntoResponse {
...
pub async fn webhook_agent(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<clawreform_types::webhook::AgentHookPayload>,
) -> impl IntoResponse {
    // Check if webhook triggers are enabled
    let wh_config = match &state.kernel.config.webhook_triggers {
        Some(c) if c.enabled => c,
...
pub async fn add_binding(
    State(state): State<Arc<AppState>>,
    Json(binding): Json<clawreform_types::config::AgentBinding>,
) -> impl IntoResponse {
...
pub async fn pairing_request(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match state.kernel.pairing.create_pairing_request() {
        Ok(req) => {
            let qr_uri = format!("clawreform://pair?token={}", req.token);
            Json(serde_json::json!({
                "token": req.token,
                "qr_uri": qr_uri,
...
pub async fn pairing_complete(
    ...
    let push_token = body
        ...
        .and_then(|v| v.as_str())
        .map(String::from);
    let device_info = clawreform_kernel::pairing::PairedDevice {
