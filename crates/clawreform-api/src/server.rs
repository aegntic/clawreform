//! ClawReform daemon server — boots the kernel and serves the HTTP API.
...
/// Daemon info written to `~/.clawreform/daemon.json` so the CLI can find us.
...
///
/// This is extracted from `run_daemon()` so that embedders (e.g. clawreform-desktop)
...
pub async fn build_router(
    ...
    let app = Router::new()
        ...
        .route(
            "/branding/{*path}",
            axum::routing::get(webchat::branding_asset),
        )
        .route(
            "/api/metrics",
...
/// Start the ClawReform daemon: boot kernel + HTTP API server.
///
...
pub async fn run_daemon(
    ...
    info!("ClawReform API server listening on http://{addr}");
    ...
    info!("ClawReform daemon stopped");
    Ok(())
}
