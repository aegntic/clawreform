// ClawReform Agents Page — Multi-step spawn wizard, detail view with tabs, file editor, personality presets
'use strict';
...
function agentsPage() {
  return {
    ...
    async loadSpawnProfiles() {
      ...
      try {
        var data = await ClawReformAPI.get('/api/profiles');
    ...
    async loadTemplates() {
      ...
      try {
        var results = await Promise.all([
          ClawReformAPI.get('/api/templates'),
          ClawReformAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    killAgent(agent) {
      var self = this;
      ClawReformToast.confirm('Stop Agent', 'Stop agent "' + agent.name + '"? The agent will be shut down.', async function() {
        try {
          await ClawReformAPI.del('/api/agents/' + agent.id);
          ClawReformToast.success('Agent "' + agent.name + '" stopped');
          self.showDetailModal = false;
          await Alpine.store('app').refreshAgents();
        } catch(e) {
          ClawReformToast.error('Failed to stop agent: ' + e.message);
        }
      });
    },
    ...
    killAllAgents() {
      ...
      ClawReformToast.confirm('Stop All Agents', 'Stop ' + list.length + ' agent(s)? All agents will be shut down.', async function() {
        var errors = [];
        for (var i = 0; i < list.length; i++) {
          try {
            await ClawReformAPI.del('/api/agents/' + list[i].id);
          } catch(e) { errors.push(list[i].name + ': ' + e.message); }
        }
        await Alpine.store('app').refreshAgents();
        if (errors.length) {
          ClawReformToast.error('Some agents failed to stop: ' + errors.join(', '));
        } else {
          ClawReformToast.success(list.length + ' agent(s) stopped');
        }
      });
    },
    ...
    async setMode(agent, mode) {
      try {
        await ClawReformAPI.put('/api/agents/' + agent.id + '/mode', { mode: mode });
        agent.mode = mode;
        ClawReformToast.success('Mode set to ' + mode);
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        ClawReformToast.error('Failed to set mode: ' + e.message);
      }
    },
    ...
    async spawnAgent() {
      ...
      if (!toml.trim()) {
        this.spawning = false;
        ClawReformToast.warn('Manifest is empty \u2014 enter agent config first');
        return;
      }

      try {
        var res = await ClawReformAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ...
          if (Object.keys(patchBody).length) {
            ClawReformAPI.patch('/api/agents/' + res.agent_id + '/config', patchBody).catch(function(e) { console.warn('Post-spawn config patch failed:', e.message); });
          }
          if (this.soulContent.trim()) {
            ClawReformAPI.put('/api/agents/' + res.agent_id + '/files/SOUL.md', { content: '# Soul\n' + this.soulContent }).catch(function(e) { console.warn('SOUL.md write failed:', e.message); });
          }
          ...
          this.spawnToml = '';
          this.spawnStep = 1;
          ClawReformToast.success('Agent "' + (res.name || 'new') + '" spawned');
          await Alpine.store('app').refreshAgents();
          this.chatWithAgent({ id: res.agent_id, name: res.name, model_provider: '?', model_name: '?' });
        } else {
          ClawReformToast.error('Spawn failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        ClawReformToast.error('Failed to spawn agent: ' + e.message);
      }
      this.spawning = false;
    },
    ...
    async loadAgentFiles() {
      ...
      try {
        var data = await ClawReformAPI.get('/api/agents/' + this.detailAgent.id + '/files');
        this.agentFiles = data.files || [];
      } catch(e) {
        this.agentFiles = [];
        ClawReformToast.error('Failed to load files: ' + e.message);
      }
    ...
    async openFile(file) {
      ...
      try {
        var data = await ClawReformAPI.get('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(file.name));
        this.editingFile = file.name;
        this.fileContent = data.content || '';
      } catch(e) {
        ClawReformToast.error('Failed to read file: ' + e.message);
      }
    },
    ...
    async saveFile() {
      ...
      try {
        await ClawReformAPI.put('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(this.editingFile), { content: this.fileContent });
        ClawReformToast.success(this.editingFile + ' saved');
        await this.loadAgentFiles();
      } catch(e) {
        ClawReformToast.error('Failed to save file: ' + e.message);
      }
    ...
    async saveConfig() {
      ...
      try {
        await ClawReformAPI.patch('/api/agents/' + this.detailAgent.id + '/config', this.configForm);
        ClawReformToast.success('Config updated');
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        ClawReformToast.error('Failed to save config: ' + e.message);
      }
    ...
    async cloneAgent(agent) {
      ...
      try {
        var res = await ClawReformAPI.post('/api/agents/' + agent.id + '/clone', { new_name: newName });
        if (res.agent_id) {
          ClawReformToast.success('Cloned as "' + res.name + '"');
          await Alpine.store('app').refreshAgents();
          this.showDetailModal = false;
        }
      } catch(e) {
        ClawReformToast.error('Clone failed: ' + e.message);
      }
    },
    ...
    async spawnFromTemplate(name) {
      try {
        var data = await ClawReformAPI.get('/api/templates/' + encodeURIComponent(name));
        if (data.manifest_toml) {
          var res = await ClawReformAPI.post('/api/agents', { manifest_toml: data.manifest_toml });
          if (res.agent_id) {
            ClawReformToast.success('Agent "' + (res.name || name) + '" spawned from template');
            await Alpine.store('app').refreshAgents();
            this.chatWithAgent({ id: res.agent_id, name: res.name || name, model_provider: '?', model_name: '?' });
          }
        }
      } catch(e) {
        ClawReformToast.error('Failed to spawn from template: ' + e.message);
      }
    },
    ...
    async spawnBuiltin(t) {
      ...
      try {
        var res = await ClawReformAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ClawReformToast.success('Agent "' + t.name + '" spawned');
          await Alpine.store('app').refreshAgents();
