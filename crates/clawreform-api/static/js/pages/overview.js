// ClawReform Overview Dashboard — Landing page with system stats + provider status
'use strict';
...
function overviewPage() {
  return {
    ...
    async loadHealth() {
      try {
        this.health = await ClawReformAPI.get('/api/health');
      } catch(e) { this.health = { status: 'unreachable' }; }
    },

    async loadStatus() {
      try {
        this.status = await ClawReformAPI.get('/api/status');
    ...
    async loadAudit() {
      try {
        var data = await ClawReformAPI.get('/api/audit/recent?n=8');
    ...
    async loadChannels() {
      try {
        var data = await ClawReformAPI.get('/api/channels');
