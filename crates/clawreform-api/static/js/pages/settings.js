// ClawReform Settings Page — Provider Hub, Model Catalog, Config, Tools + Security, Network, Migration tabs
'use strict';
  ...
  return {
    ...
    configurableFeatures: [
      ...
      },
      {
        ...
        configHint: 'Set api_key in ~/.clawreform/config.toml for remote access. Empty = localhost only.',
        valueKey: 'auth'
      }
    ],
    ...
    async loadSysInfo() {
      try {
        var ver = await ClawReformAPI.get('/api/version');
        var status = await ClawReformAPI.get('/api/status');
        this.sysInfo = {
          version: ver.version || '-',
    ...
    async loadUsage() {
      try {
        var data = await ClawReformAPI.get('/api/usage');
    ...
    async loadTools() {
      try {
        var data = await ClawReformAPI.get('/api/tools');
    ...
    async loadConfig() {
      try {
        this.config = await ClawReformAPI.get('/api/config');
      } catch(e) { this.config = {}; }
    },
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          ClawReformAPI.get('/api/config/schema').catch(function() { return {}; }),
          ClawReformAPI.get('/api/config')
        ]);
    ...
    async saveConfigField(section, field, value) {
      ...
      try {
        await ClawReformAPI.post('/api/config/set', { path: key, value: value });
        this.configDirty[key] = false;
        ClawReformToast.success('Saved ' + key);
      } catch(e) {
        ClawReformToast.error('Failed to save: ' + e.message);
      }
    ...
    async saveProviderKey(provider) {
      ...
      if (!key || !key.trim()) { ClawReformToast.error('Please enter an API key'); return; }
      try {
        await ClawReformAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key.trim() });
        ClawReformToast.success('API key saved for ' + provider.display_name);
        this.providerKeyInputs[provider.id] = '';
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        ClawReformToast.error('Failed to save key: ' + e.message);
      }
    },

    async removeProviderKey(provider) {
      try {
        await ClawReformAPI.del('/api/providers/' + encodeURIComponent(provider.id) + '/key');
        ClawReformToast.success('API key removed for ' + provider.display_name);
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        ClawReformToast.error('Failed to remove key: ' + e.message);
      }
    },
    ...
    async testProvider(provider) {
      ...
      try {
        var result = await ClawReformAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/test', {});
        this.providerTestResults[provider.id] = result;
        if (result.status === 'ok') {
          ClawReformToast.success(provider.display_name + ' connected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          ClawReformToast.error(provider.display_name + ': ' + (result.error || 'Connection failed'));
        }
      } catch(e) {
        this.providerTestResults[provider.id] = { status: 'error', error: e.message };
        ClawReformToast.error('Test failed: ' + e.message);
      }
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { ClawReformToast.error('Please enter a base URL'); return; }
      url = url.trim();
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        ClawReformToast.error('URL must start with http:// or https://'); return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await ClawReformAPI.put('/api/providers/' + encodeURIComponent(provider.id) + '/url', { base_url: url });
        if (result.reachable) {
          ClawReformToast.success(provider.display_name + ' URL saved &mdash; reachable (' + (result.latency_ms || '?') + 'ms)');
        } else {
          ClawReformToast.warning(provider.display_name + ' URL saved but not reachable');
        }
        await this.loadProviders();
      } catch(e) {
        ClawReformToast.error('Failed to save URL: ' + e.message);
      }
    ...
    async loadSecurity() {
      ...
      try {
        this.securityData = await ClawReformAPI.get('/api/security');
      } catch(e) {
    ...
    async verifyAuditChain() {
      ...
      try {
        var res = await ClawReformAPI.get('/api/audit/verify');
    ...
    async loadPeers() {
      ...
      this.peersLoadError = '';
      try {
        var data = await ClawReformAPI.get('/api/peers');
    ...
    async autoDetect() {
      this.detecting = true;
      try {
        var data = await ClawReformAPI.get('/api/migrate/detect');
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await ClawReformAPI.post('/api/migrate/scan', { path: this.sourcePath });
        if (data.error) {
          ClawReformToast.error('Scan error: ' + data.error);
          this.scanning = false;
          return;
        }
        ...
        ClawReformToast.error('Scan failed: ' + e.message);
      }
      this.scanning = false;
    },
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        if (!target) target = '';
        var data = await ClawReformAPI.post('/api/migrate', {
          source: 'openclaw',
