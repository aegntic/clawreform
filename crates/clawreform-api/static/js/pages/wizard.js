// ClawReform Setup Wizard — First-run guided setup (Provider + Agent + Channel)
'use strict';
...
function wizardPage() {
  return {
    ...
    async sendTryItMessage(text) {
      ...
      try {
        var res = await ClawReformAPI.post('/api/agents/' + this.createdAgent.id + '/message', { message: text });
    ...
    async saveKey() {
      ...
      if (!key) {
        ClawReformToast.error('Please enter an API key');
        return;
      }
      this.savingKey = true;
      try {
        await ClawReformAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key });
        this.apiKeyInput = '';
        this.keySaved = true;
    ...
    async createAgent() {
      ...
      if (!name) {
        ClawReformToast.error('Please enter a name for your agent');
        return;
      }
      ...
      try {
        ...
        if (res.agent_id) {
          ...
          ClawReformToast.success('Agent "' + (res.name || name) + '" created');
          await Alpine.store('app').refreshAgents();
        } else {
          ClawReformToast.error('Failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        ClawReformToast.error('Failed to create agent: ' + e.message);
      }
    ...
    async configureChannel() {
      ...
      if (!token) {
        ClawReformToast.error('Please enter the ' + ch.token_label);
        return;
      }
      ...
      try {
        ...
        fields.token = token;
        await ClawReformAPI.post('/api/channels/' + ch.name + '/configure', { fields: fields });
        this.channelConfigured = true;
        this.setupSummary.channel = ch.display_name;
        ClawReformToast.success(ch.display_name + ' configured and activated.');
      } catch(e) {
        ClawReformToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
