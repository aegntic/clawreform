...
\x1b[1;36mExamples:\x1b[0m
  clawreform init                 Initialize config and data directories
  clawreform start                Start the kernel daemon
  clawreform tui                  Launch the interactive terminal dashboard
  clawreform chat                 Quick chat with the default agent
  clawreform agent new coder      Spawn a new agent from a template
  clawreform models list          Browse available LLM models
  clawreform add github           Install the GitHub integration
  clawreform doctor               Run diagnostic health checks
  clawreform channel setup        Interactive channel setup wizard
  clawreform cron list            List scheduled jobs

\x1b[1;36mQuick Start:\x1b[0m
  1. clawreform init              Set up config + API key
  2. clawreform start             Launch the daemon
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize ClawReform (create ~/.clawreform/ and default config).
    Init {
        /// Quick mode: no prompts, just write config + .env (for CI/scripts).
        #[arg(long)]
        quick: bool,
    },
    /// Start the ClawReform kernel daemon (API server + kernel).
    Start,
    ...
    /// Migrate from another agent framework to ClawReform.
    Migrate(MigrateArgs),
...
enum ConfigCommands {
    ...
    /// Save an API key to ~/.clawreform/.env (prompts interactively).
    SetKey {
...
fn main() {
    // Load ~/.clawreform/.env into process environment (system env takes priority).
    dotenv::load_dotenv();
...
pub(crate) fn find_daemon() -> Option<String> {
    let home_dir = dirs::home_dir()?.join(".clawreform");
    ...
    match resp {
        Ok(r) => {
            ...
            if status.is_server_error() {
                ui::error_with_fix(
                    ...
                    "Check daemon logs: ~/.clawreform/tui.log",
                );
            }
            body
        }
        Err(e) => {
            ...
            if msg.contains("timed out") || msg.contains("Timeout") {
                ui::error_with_fix(
                    "Request timed out",
                    "The agent may be processing a complex request. Try again, or check `clawreform status`",
                );
            } else if msg.contains("Connection refused") || msg.contains("connect") {
                ui::error_with_fix(
                    "Cannot connect to daemon",
                    "Is the daemon running? Start it with: clawreform start",
                );
            } else {
                ui::error_with_fix(
                    &format!("Daemon communication error: {msg}"),
                    "Check `clawreform status` or restart: clawreform start",
                );
            }
            std::process::exit(1);
        }
...
fn cmd_init(quick: bool) {
    ...
    let clawreform_dir = home.join(".clawreform");

    // --- Ensure directories exist ---
    if !clawreform_dir.exists() {
        std::fs::create_dir_all(&clawreform_dir).unwrap_or_else(|e| {
            ui::error_with_fix(
                &format!("Failed to create {}", clawreform_dir.display()),
                &format!("Check permissions on {}", home.display()),
            );
            eprintln!("  {e}");
            std::process::exit(1);
        });
        restrict_dir_permissions(&clawreform_dir);
    }

    for sub in ["data", "agents"] {
        let dir = clawreform_dir.join(sub);
        if !dir.exists() {
    ...
    bundled_agents::install_bundled_agents(&clawreform_dir.join("agents"));

    if quick {
        cmd_init_quick(&clawreform_dir);
    } else {
        cmd_init_interactive(&clawreform_dir);
    }
}

/// Quick init: no prompts, auto-detect, write config + .env, print next steps.
fn cmd_init_quick(clawreform_dir: &std::path::Path) {
    ui::banner();
    ui::blank();
    ...
    ui::blank();
    ui::next_steps(&[
        "Start the daemon:  clawreform start",
        "Chat:              clawreform chat",
    ]);
}

/// Interactive 5-step onboarding wizard (ratatui TUI).
fn cmd_init_interactive(clawreform_dir: &std::path::Path) {
        ...
        InitResult::Completed {
            ...
            match launch {
                LaunchChoice::Desktop => {
                    launch_desktop_app(clawreform_dir);
                }
                LaunchChoice::Dashboard => {
                    if let Some(base) = find_daemon() {
                        ...
                        ui::error("Daemon is not running. Start it with: clawreform start");
                    }
                }
                LaunchChoice::Chat => {
...
/// Launch the clawreform-desktop Tauri app, connecting to the running daemon.
fn launch_desktop_app(_clawreform_dir: &std::path::Path) {
    ...
    let desktop_bin = {
        ...
        #[cfg(windows)]
        let name = "clawreform-desktop.exe";
        #[cfg(not(windows))]
        let name = "clawreform-desktop";
        ...
        Some(ref path) if path.exists() => {
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ...
                    ui::hint("Try: clawreform dashboard");
                }
            }
        }
        _ => {
            ui::error("Desktop app not found.");
            ui::hint("Install it with: cargo install clawreform-desktop");
...
fn cmd_start(config: Option<PathBuf>) {
    if let Some(base) = find_daemon() {
        ui::error_with_fix(
            ...
            "Use `clawreform status` to check it, or stop it first",
        );
        std::process::exit(1);
    }
    ...
    rt.block_on(async {
        let kernel = match ClawReformKernel::boot(config.as_deref()) {
            Ok(k) => k,
            Err(e) => {
                boot_kernel_error(&e);
                std::process::exit(1);
        ...
        ui::kv("Model", &model);
        ui::blank();
        ui::hint("Open the dashboard in your browser, or run `clawreform chat`");
        ui::hint("Press Ctrl+C to stop the daemon");
        ui::blank();

        if let Err(e) =
            clawreform_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
    ...
    match find_daemon() {
            ...
            match client.post(format!("{base}/api/shutdown")).send() {
                Ok(r) if r.status().is_success() => {
                    ...
                    if let Some(home) = dirs::home_dir() {
                        let of_dir = home.join(".clawreform");
        ...
        None => {
            ui::warn_with_fix(
                ...
                "Is it running? Check with: clawreform status",
            );
        }
    }
}
...
fn boot_kernel_error(e: &clawreform_kernel::error::KernelError) {
    let msg = e.to_string();
    if msg.contains("parse") || msg.contains("toml") || msg.contains("config") {
        ui::error_with_fix(
            "Failed to parse configuration",
            "Check your config.toml syntax: clawreform config show",
        );
        ...
        ui::error_with_fix(
            ...
            "Run `clawreform doctor` to diagnose the issue",
        );
    }
}
...
fn cmd_agent_spawn(config: Option<PathBuf>, manifest_path: PathBuf) {
    if !manifest_path.exists() {
        ui::error_with_fix(
            ...
            "Use `clawreform agent new` to spawn from a template instead",
        );
        std::process::exit(1);
    }
    ...
    if let Some(base) = find_daemon() {
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                println!("  For persistent agents, use `clawreform start` first.");
            }
            Err(e) => {
...
fn cmd_agent_new(config: Option<PathBuf>, template_name: Option<String>) {
    ...
    if all_templates.is_empty() {
        ui::error_with_fix(
            ...
            "Run `clawreform init` to set up the agents directory",
        );
        std::process::exit(1);
    }
        ...
        Some(ref name) => match all_templates.iter().find(|t| t.name == *name) {
            ...
            None => {
                ui::error_with_fix(
                    ...
                    "Run `clawreform agent new` to see available templates",
                );
                std::process::exit(1);
            }
        },
...
fn spawn_template_agent(config: Option<PathBuf>, template: &templates::AgentTemplate) {
    if let Some(base) = find_daemon() {
        ...
        if let Some(id) = body["agent_id"].as_str() {
            ...
            ui::blank();
            ui::hint(&format!("Chat: clawreform chat {}", template.name));
        } else {
            ui::error(&format!(
                "Failed to spawn: {}",
                body["error"].as_str().unwrap_or("Unknown error")
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                ui::hint("For persistent agents, use `clawreform start` first");
            }
            Err(e) => {
...
fn cmd_status(config: Option<PathBuf>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::blank();
        ui::hint("Run `clawreform start` to launch the daemon");
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if let Some(h) = &home {
        let clawreform_dir = h.join(".clawreform");
        ...
        if clawreform_dir.exists() {
            ...
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                if std::fs::create_dir_all(&clawreform_dir).is_ok() {
                    restrict_dir_permissions(&clawreform_dir);
                    for sub in ["data", "agents"] {
        ...
        let env_path = clawreform_dir.join(".env");
        if env_path.exists() {
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt;
            ...
            if !json {
                ui::check_warn(
                    ".env file not found (create with: clawreform config set-key <provider>)",
                );
            }
            ...
            match toml::from_str::<toml::Value>(&config_content) {
                ...
                Err(e) => {
                    if !json {
                        ...
                        ui::hint("Fix with: clawreform config edit");
                    }
...
"#,
        ...
        if let Some(ref base) = daemon_running {
            ...
            if !json {
                ui::check_warn("Daemon not running (start with `clawreform start`)");
            }
        ...
        let daemon_json_path = clawreform_dir.join("daemon.json");
        ...
        let db_path = clawreform_dir.join("data").join("clawreform.db");
        if db_path.exists() {
        ...
        #[cfg(unix)]
        {
            if let Ok(output) = std::process::Command::new("df")
                .args(["-m", &clawreform_dir.display().to_string()])
                .output()
            {
        ...
        let agents_dir = clawreform_dir.join("agents");
    ...
    if !any_key_set {
        if !json {
            ...
            ui::blank();
            ui::hint("Or run: clawreform config set-key groq");
        }
        all_ok = false;
    }
    ...
    if let Some(ref h) = home {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<clawreform_types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = clawreform_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
                    ...
                    if !cfg.mcp_servers.is_empty() {
                        ...
                        for server in &cfg.mcp_servers {
                            ...
                            match &server.transport {
                                clawreform_types::config::McpTransportEntry::Stdio {
                                    command,
                                    ..
                                } => {
                                    if command.is_empty() {
                                ...
                                clawreform_types::config::McpTransportEntry::Sse { url } => {
                                    if url.is_empty() {
                                        if !json {
                                            ui::check_warn(&format!(
    ...
    // --- Check 13: Skill registry health ---
    {
        ...
        let skills_dir = home
            .as_ref()
            .map(|h| h.join(".clawreform").join("skills"))
            .unwrap_or_else(|| std::path::PathBuf::from("skills"));
        let mut skill_reg = clawreform_skills::registry::SkillRegistry::new(skills_dir.clone());
    ...
    if let Some(ref h) = home {
        ...
        let mut ext_registry =
            clawreform_extensions::registry::IntegrationRegistry::new(&clawreform_dir);
    ...
    if json {
        ...
        if all_ok {
            ...
            if !repair {
                ui::hint("Run `clawreform doctor --repair` to attempt auto-fix");
            }
        }
    }
}
        ...
        match start_daemon_background() {
            ...
            Err(e) => {
                ui::error_with_fix(
                    ...
                    "Start it manually: clawreform start",
                );
                std::process::exit(1);
            }
        }
...
fn cmd_completion(shell: clap_complete::Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "clawreform", &mut std::io::stdout());
}
...
fn require_daemon(command: &str) -> String {
    find_daemon().unwrap_or_else(|| {
        ui::error_with_fix(
            &format!("`clawreform {command}` requires a running daemon"),
            "Start the daemon: clawreform start",
        );
        ui::hint("Or try `clawreform chat` which works without a daemon");
        std::process::exit(1);
    })
}

fn boot_kernel(config: Option<PathBuf>) -> ClawReformKernel {
    match ClawReformKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
            boot_kernel_error(&e);
            std::process::exit(1);
...
fn cmd_migrate(args: MigrateArgs) {
    let source = match args.from {
        MigrateSourceArg::Openclaw => clawreform_migrate::MigrateSource::OpenClaw,
        MigrateSourceArg::Langchain => clawreform_migrate::MigrateSource::LangChain,
        MigrateSourceArg::Autogpt => clawreform_migrate::MigrateSource::AutoGpt,
    };
    ...
    let source_dir = args.source_dir.unwrap_or_else(|| {
        ...
        match source {
            clawreform_migrate::MigrateSource::OpenClaw => home.join(".openclaw"),
            clawreform_migrate::MigrateSource::LangChain => home.join(".langchain"),
            clawreform_migrate::MigrateSource::AutoGpt => home.join("Auto-GPT"),
        }
    });
    ...
    let options = clawreform_migrate::MigrateOptions {
        source,
        source_dir,
        target_dir,
    ...
    match clawreform_migrate::run_migration(&options) {
        Ok(report) => {
...
fn cmd_skill_install(source: &str) {
    ...
    if source_path.exists() && source_path.is_dir() {
        ...
        if !manifest_path.exists() {
            ...
            if clawreform_skills::openclaw_compat::detect_openclaw_skill(&source_path) {
                println!("Detected OpenClaw skill format. Converting...");
                match clawreform_skills::openclaw_compat::convert_openclaw_skill(&source_path) {
                    Ok(manifest) => {
        ...
        let manifest: clawreform_skills::SkillManifest =
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let client = clawreform_skills::marketplace::MarketplaceClient::new(
            clawreform_skills::marketplace::MarketplaceConfig::default(),
        );
...
fn cmd_skill_list() {
    ...
    let mut registry = clawreform_skills::registry::SkillRegistry::new(skills_dir);
...
"#
    ...
    println!("  2. Test locally: clawreform skill test");
    println!(
        "  3. Install: clawreform skill install {}",
        skill_dir.display()
    );
}
...
fn cmd_channel_list() {
    ...
    if !config_path.exists() {
        println!("No configuration found. Run `clawreform init` first.");
        return;
    }
    ...
    println!("\nUse `clawreform channel setup <channel>` to configure a channel.");
}
    ...
    match channel.as_str() {
        "telegram" => {
            ...
            match dotenv::save_env_key("TELEGRAM_BOT_TOKEN", &token) {
                Ok(()) => ui::success("Token saved to ~/.clawreform/.env"),
            ...
            for (key, val) in [
                ...
                if !val.is_empty() {
                    match dotenv::save_env_key(key, val) {
                        Ok(()) => ui::success(&format!("{key} saved to ~/.clawreform/.env")),
        ...
        "email" => {
            ...
            if !password.is_empty() {
                match dotenv::save_env_key("EMAIL_PASSWORD", &password) {
                    Ok(()) => ui::success("Password saved to ~/.clawreform/.env"),
        ...
        "signal" => {
            ...
            if !phone.is_empty() {
                match dotenv::save_env_key("SIGNAL_PHONE", &phone) {
                    Ok(()) => ui::success("Phone saved to ~/.clawreform/.env"),
        ...
        "matrix" => {
            ...
            println!("     (e.g., register @clawreform-bot:matrix.org)");
            println!("  2. Obtain an access token:");
            println!("     curl -X POST https://matrix.org/_matrix/client/r0/login \\");
            println!("       -d '{{\"type\":\"m.login.password\",\"user\":\"clawreform-bot\",\"password\":\"...\"}}'");
...
fn maybe_write_channel_config(channel: &str, config_block: &str) {
    ...
    if !config_path.exists() {
        ui::hint("No config.toml found. Run `clawreform init` first.");
        return;
    }
...
fn cmd_channel_test(channel: &str) {
    if let Some(base) = find_daemon() {
        ...
        eprintln!("Channel test requires a running daemon. Start with: clawreform start");
        std::process::exit(1);
    }
}
...
fn cmd_channel_toggle(channel: &str, enable: bool) {
    ...
    if let Some(base) = find_daemon() {
        ...
        println!("Edit ~/.clawreform/config.toml to persist this change.");
    }
}
...
/// Spawn `clawreform start` as a detached background process.
///
...
fn cmd_config_show() {
    ...
    if !config_path.exists() {
        ...
        println!("Run `clawreform init` to create one.");
        return;
    }
...
fn cmd_config_get(key: &str) {
    ...
    if !config_path.exists() {
        ui::error_with_fix("No config file found", "Run `clawreform init` first");
        std::process::exit(1);
    }
    ...
    let table: toml::Value = toml::from_str(&content).unwrap_or_else(|e| {
        ui::error_with_fix(
            ...
            "Fix your config.toml syntax, or run `clawreform config edit`",
        );
        std::process::exit(1);
    });
...
fn cmd_config_set_key(provider: &str) {
    ...
    match dotenv::save_env_key(&env_var, &key) {
        Ok(()) => {
            ui::success(&format!("Saved {env_var} to ~/.clawreform/.env"));
            // Test the key
            print!("  Testing key... ");
...
fn cmd_config_delete_key(provider: &str) {
    ...
    match dotenv::remove_env_key(&env_var) {
        Ok(()) => ui::success(&format!("Removed {env_var} from ~/.clawreform/.env")),
        Err(e) => {
...
fn cmd_config_test_key(provider: &str) {
    ...
    if test_api_key(provider, &env_var) {
        ...
        ui::hint(&format!("Update key: clawreform config set-key {provider}"));
        std::process::exit(1);
    }
}
...
pub(crate) fn clawreform_home() -> PathBuf {
    dirs::home_dir()
        .unwrap_or_else(|| {
            eprintln!("Error: Could not determine home directory");
            std::process::exit(1);
...
// ---------------------------------------------------------------------------
// Integration commands (clawreform add/remove/integrations)
// ---------------------------------------------------------------------------

fn cmd_integration_add(name: &str, key: Option<&str>) {
    let home = clawreform_home();
    let mut registry = clawreform_extensions::registry::IntegrationRegistry::new(&home);
    ...
    let vault = if vault_path.exists() {
        let mut v = clawreform_extensions::vault::CredentialVault::new(vault_path);
        if v.unlock().is_ok() {
            Some(v)
        } else {
            None
    ...
    let mut resolver =
        clawreform_extensions::credentials::CredentialResolver::new(vault, Some(&dotenv_path))
    ...
    match clawreform_extensions::installer::install_integration(
        &mut registry,
        &mut resolver,
        name,
        &provided_keys,
        ...
        Ok(result) => {
            match &result.status {
                clawreform_extensions::IntegrationStatus::Ready => {
                    ui::success(&result.message);
                }
                clawreform_extensions::IntegrationStatus::Setup => {
                    println!("{}", result.message.yellow());
                    println!("\nTo add credentials:");
                    for env in &template.required_env {
                        if env.is_secret {
                            println!("  clawreform vault set {}  # {}", env.name, env.help);
...
fn cmd_integration_remove(name: &str) {
    ...
    match clawreform_extensions::installer::remove_integration(&mut registry, name) {
        Ok(msg) => {
            ui::success(&msg);
            // Hot-reload daemon
            if let Some(base_url) = find_daemon() {
...
fn cmd_integrations_list(query: Option<&str>) {
    ...
    let dotenv_path = home.join(".env");
    let resolver =
        clawreform_extensions::credentials::CredentialResolver::new(None, Some(&dotenv_path));

    let entries = if let Some(q) = query {
        clawreform_extensions::installer::search_integrations(&registry, q)
    } else {
        clawreform_extensions::installer::list_integrations(&registry, &resolver)
    };
    ...
    let mut by_category: std::collections::BTreeMap<
        String,
        Vec<&clawreform_extensions::installer::IntegrationListEntry>,
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let status_badge = match &item.status {
                clawreform_extensions::IntegrationStatus::Ready => "[Ready]".green().to_string(),
                clawreform_extensions::IntegrationStatus::Setup => "[Setup]".yellow().to_string(),
                clawreform_extensions::IntegrationStatus::Available => {
                    "[Available]".dimmed().to_string()
                }
                clawreform_extensions::IntegrationStatus::Error(msg) => {
                    format!("[Error: {msg}]").red().to_string()
                }
                clawreform_extensions::IntegrationStatus::Disabled => {
    ...
    println!("  Use `clawreform add <name>` to install an integration.");
}

// ---------------------------------------------------------------------------
// Vault commands (clawreform vault init/set/list/remove)
// ---------------------------------------------------------------------------

fn cmd_vault_init() {
    let home = clawreform_home();
    let vault_path = home.join("vault.enc");
    let mut vault = clawreform_extensions::vault::CredentialVault::new(vault_path);
...
fn cmd_vault_set(key: &str) {
    ...
    if !vault.exists() {
        ui::error("Vault not initialized. Run: clawreform vault init");
        std::process::exit(1);
    }
...
fn cmd_vault_list() {
    ...
    if !vault.exists() {
        println!("Vault not initialized. Run: clawreform vault init");
        return;
    }
...
// ---------------------------------------------------------------------------
// Scaffold commands (clawreform new skill/integration)
// ---------------------------------------------------------------------------
...
fn cmd_scaffold(kind: ScaffoldKind) {
    ...
    let result = match kind {
        ScaffoldKind::Skill => {
            clawreform_extensions::installer::scaffold_skill(&cwd.join("my-skill"))
        }
        ScaffoldKind::Integration => {
            clawreform_extensions::installer::scaffold_integration(&cwd.join("my-integration"))
        }
    };
    match result {
...
fn cmd_models_list(provider_filter: Option<&str>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        let catalog = clawreform_runtime::model_catalog::ModelCatalog::new();
...
fn cmd_logs(lines: usize, follow: bool) {
    let log_path = dirs::home_dir()
        .map(|h| h.join(".clawreform").join("tui.log"))
...
fn cmd_health(json: bool) {
    match find_daemon() {
        ...
        None => {
            ...
            ui::hint("Start it with: clawreform start");
            std::process::exit(1);
        }
    }
}
...
fn cmd_system_info(json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::kv("Version", env!("CARGO_PKG_VERSION"));
        ui::kv_warn("Daemon", "NOT RUNNING");
        ui::hint("Start with: clawreform start");
    }
}
...
fn cmd_system_version(json: bool) {
    ...
    println!("clawreform {}", env!("CARGO_PKG_VERSION"));
}

fn cmd_reset(confirm: bool) {
    let clawreform_dir = match dirs::home_dir() {
        Some(h) => h.join(".clawreform"),
        None => {
            ui::error("Could not determine home directory");
            std::process::exit(1);
        }
...
mod tests {
    ...
    fn test_doctor_skill_registry_loads_bundled() {
        let skills_dir = std::env::temp_dir().join("clawreform-doctor-test-skills");
        let mut skill_reg = clawreform_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn test_doctor_extension_registry_loads_bundled() {
        let tmp = std::env::temp_dir().join("clawreform-doctor-test-ext");
    ...
    fn test_doctor_config_deser_default() {
        ...
        let parsed: clawreform_types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
api_key_env = "GROQ_API_KEY"
"#;
        let config: clawreform_types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            clawreform_types::config::ExecSecurityMode::Allowlist
        );
...
"#;
        ...
        match &config.mcp_servers[0].transport {
            clawreform_types::config::McpTransportEntry::Stdio { command, args } => {
    ...
    fn test_doctor_skill_injection_scan_clean() {
        ...
        let warnings = clawreform_skills::verify::SkillVerifier::scan_prompt_content(clean_content);
