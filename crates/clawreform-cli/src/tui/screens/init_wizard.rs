...
//!
//! Launched by `clawreform init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use clawreform_runtime::model_catalog::ModelCatalog;
use clawreform_types::model_catalog::ModelTier;
...
struct State {
    ...
    openclaw_scan: Option<clawreform_migrate::openclaw::ScanResult>,
    migration_report: Option<clawreform_migrate::report::MigrationReport>,
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<clawreform_migrate::report::MigrationReport, String>>();
        ...
        MigrationPhase::Offer => match code {
            ...
            KeyCode::Enter => {
                ...
                if yes {
                    ...
                    let target_dir = home.join(".clawreform");
                    let tx = migrate_tx.clone();
                    std::thread::spawn(move || {
                        let options = clawreform_migrate::MigrateOptions {
                            source: clawreform_migrate::MigrateSource::OpenClaw,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result =
                            clawreform_migrate::run_migration(&options).map_err(|e| format!("{e}"));
...
fn save_config(state: &mut State) {
    ...
    let _ = std::fs::create_dir_all(clawreform_dir.join("agents"));
    let _ = std::fs::create_dir_all(clawreform_dir.join("data"));
    crate::restrict_dir_permissions(&clawreform_dir);
...
/// Check if the `clawreform-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "ClawReform Init  Step X of 7"
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use clawreform_migrate::report::ItemKind;
...
fn draw_complete(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  How do you want to use ClawReform?",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),
