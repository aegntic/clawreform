    ...
    if state.detecting {
        ...
        if let Some((provider, env_var)) = detect_provider() {
            ...
            status_lines.push(Line::from(vec![Span::styled(
                "  Run 'clawreform init' to get started",
                theme::hint_style(),
            )]));
        }
