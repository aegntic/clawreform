//! ClawReformKernel — assembles all subsystems and provides the main API.
...
use clawreform_memory::MemorySubstrate;
use clawreform_runtime::agent_loop::{run_agent_loop, run_agent_loop_streaming, AgentLoopResult};
use clawreform_runtime::audit::AuditLog;
use clawreform_runtime::drivers;
use clawreform_runtime::kernel_handle::{self, KernelHandle};
use clawreform_runtime::llm_driver::{CompletionRequest, DriverConfig, LlmDriver, StreamEvent};
use clawreform_runtime::python_runtime::{self, PythonConfig};
use clawreform_runtime::routing::ModelRouter;
use clawreform_runtime::sandbox::{SandboxConfig, WasmSandbox};
use clawreform_runtime::tool_runner::builtin_tool_definitions;
use clawreform_types::agent::*;
use clawreform_types::capability::Capability;
use clawreform_types::config::KernelConfig;
use clawreform_types::error::ClawReformError;
use clawreform_types::event::*;
use clawreform_types::memory::Memory;
use clawreform_types::tool::ToolDefinition;
...
/// The main ClawReform kernel — coordinates all subsystems.
pub struct ClawReformKernel {
    ...
    pub model_catalog: std::sync::RwLock<clawreform_runtime::model_catalog::ModelCatalog>,
    /// Skill registry for plugin skills (RwLock for hot-reload on install/uninstall).
    pub skill_registry: std::sync::RwLock<clawreform_skills::registry::SkillRegistry>,
    ...
    pub mcp_connections: tokio::sync::Mutex<Vec<clawreform_runtime::mcp::McpConnection>>,
    ...
    pub a2a_task_store: clawreform_runtime::a2a::A2aTaskStore,
    /// Discovered external A2A agent cards.
    pub a2a_external_agents: std::sync::Mutex<Vec<(String, clawreform_runtime::a2a::AgentCard)>>,
    /// Web tools context (multi-provider search + SSRF-protected fetch + caching).
    pub web_ctx: clawreform_runtime::web_search::WebToolsContext,
    /// Browser automation manager (Playwright bridge sessions).
    pub browser_ctx: clawreform_runtime::browser::BrowserManager,
    /// Media understanding engine (image description, audio transcription).
    pub media_engine: clawreform_runtime::media_understanding::MediaEngine,
    /// Text-to-speech engine.
    pub tts_engine: clawreform_runtime::tts::TtsEngine,
    ...
    pub embedding_driver:
        Option<Arc<dyn clawreform_runtime::embedding::EmbeddingDriver + Send + Sync>>,
    /// Hand registry — curated autonomous capability packages.
    pub hand_registry: clawreform_hands::registry::HandRegistry,
    /// Extension/integration registry (bundled MCP templates + install state).
    pub extension_registry: std::sync::RwLock<clawreform_extensions::registry::IntegrationRegistry>,
    ...
    pub bindings: std::sync::Mutex<Vec<clawreform_types::config::AgentBinding>>,
    /// Broadcast configuration.
    pub broadcast: clawreform_types::config::BroadcastConfig,
    /// Auto-reply engine.
    pub auto_reply_engine: crate::auto_reply::AutoReplyEngine,
    /// Plugin lifecycle hook registry.
    pub hooks: clawreform_runtime::hooks::HookRegistry,
    /// Persistent process manager for interactive sessions (REPLs, servers).
    pub process_manager: Arc<clawreform_runtime::process_manager::ProcessManager>,
    /// OFP peer registry — tracks connected peers.
    pub peer_registry: Option<clawreform_wire::PeerRegistry>,
    /// OFP peer node — the local networking node.
    pub peer_node: Option<Arc<clawreform_wire::PeerNode>>,
...
pub struct DeliveryTracker {
    receipts: dashmap::DashMap<AgentId, Vec<clawreform_channels::types::DeliveryReceipt>>,
}
...
impl DeliveryTracker {
    ...
    pub fn record(&self, agent_id: AgentId, receipt: clawreform_channels::types::DeliveryReceipt) {
    ...
    pub fn get_receipts(
        ...
        agent_id: AgentId,
        limit: usize,
    ) -> Vec<clawreform_channels::types::DeliveryReceipt> {
        self.receipts
            .get(&agent_id)
    ...
    pub fn sent_receipt(
        channel: &str,
        recipient: &str,
    ) -> clawreform_channels::types::DeliveryReceipt {
        clawreform_channels::types::DeliveryReceipt {
            message_id: uuid::Uuid::new_v4().to_string(),
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: clawreform_channels::types::DeliveryStatus::Sent,
    ...
    pub fn failed_receipt(
        ...
        clawreform_channels::types::DeliveryReceipt {
            ...
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: clawreform_channels::types::DeliveryStatus::Failed,
            timestamp: chrono::Utc::now(),
...
impl ClawReformKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        use clawreform_types::config::KernelMode;
        ...
        match config.mode {
            KernelMode::Stable => {
                info!("Booting ClawReform kernel in STABLE mode — conservative defaults enforced");
            }
            KernelMode::Dev => {
                warn!("Booting ClawReform kernel in DEV mode — experimental features enabled");
            }
            KernelMode::Default => {
                info!("Booting ClawReform kernel...");
            }
        }
        ...
        let db_path = config
            ...
            .sqlite_path
            .clone()
            .unwrap_or_else(|| config.data_dir.join("clawreform.db"));
        let memory = Arc::new(
        ...
        let driver: Arc<dyn LlmDriver> = if !config.fallback_providers.is_empty() {
            ...
            if chain.len() > 1 {
                Arc::new(clawreform_runtime::drivers::fallback::FallbackDriver::new(
                    chain,
                ))
            } else {
                primary_driver
        ...
        let metering = Arc::new(MeteringEngine::new(Arc::new(
            clawreform_memory::usage::UsageStore::new(memory.usage_conn()),
        )));
        ...
        let mut model_catalog = clawreform_runtime::model_catalog::ModelCatalog::new();
        ...
        let mut skill_registry = clawreform_skills::registry::SkillRegistry::new(skills_dir);
        ...
        let mut hand_registry = clawreform_hands::registry::HandRegistry::new();
        ...
        let mut extension_registry =
            clawreform_extensions::registry::IntegrationRegistry::new(&config.home_dir);
        ...
        let health_config = clawreform_extensions::health::HealthMonitorConfig {
        ...
        let extension_health = clawreform_extensions::health::HealthMonitor::new(health_config);
        ...
        let web_cache = Arc::new(clawreform_runtime::web_cache::WebCache::new(cache_ttl));
        let web_ctx = clawreform_runtime::web_search::WebToolsContext {
            search: clawreform_runtime::web_search::WebSearchEngine::new(
                config.web.clone(),
                web_cache.clone(),
            ),
            fetch: clawreform_runtime::web_fetch::WebFetchEngine::new(
        ...
        let embedding_driver: Option<
            Arc<dyn clawreform_runtime::embedding::EmbeddingDriver + Send + Sync>,
        > = {
            use clawreform_runtime::embedding::create_embedding_driver;
        ...
        let browser_ctx = clawreform_runtime::browser::BrowserManager::new(config.browser.clone());

        // Initialize media understanding engine
        let media_engine =
            clawreform_runtime::media_understanding::MediaEngine::new(config.media.clone());
        let tts_engine = clawreform_runtime::tts::TtsEngine::new(config.tts.clone());
        ...
        let kernel = Self {
            ...
            a2a_task_store: clawreform_runtime::a2a::A2aTaskStore::default(),
            ...
            auto_reply_engine,
            hooks: clawreform_runtime::hooks::HookRegistry::new(),
            process_manager: Arc::new(clawreform_runtime::process_manager::ProcessManager::new(5)),
            peer_registry: None,
            peer_node: None,
        ...
        info!("ClawReform kernel booted successfully");
        Ok(kernel)
    }
    ...
    pub fn spawn_agent_with_parent(
        ...
        self.memory
            ...
            .map_err(KernelError::ClawReform)?;
        ...
        // This ensures agents respect the user's configured provider from `clawreform init`.
        ...
        self.audit_log.record(
            agent_id.to_string(),
            clawreform_runtime::audit::AuditAction::AgentSpawn,
    ...
    pub fn verify_signed_manifest(&self, signed_json: &str) -> KernelResult<String> {
        let signed: clawreform_types::manifest_signing::SignedManifest =
            serde_json::from_str(signed_json).map_err(|e| {
                KernelError::ClawReform(clawreform_types::error::ClawReformError::Config(format!(
    ...
    pub async fn send_message_with_handle(
        ...
        let entry = self.registry.get(agent_id).ok_or_else(|| {
            KernelError::ClawReform(ClawReformError::AgentNotFound(agent_id.to_string()))
        })?;
        ...
        match result {
            Ok(result) => {
                ...
                self.audit_log.record(
                    agent_id.to_string(),
                    clawreform_runtime::audit::AuditAction::AgentMessage,
                    format!(
    ...
    pub fn send_message_streaming(
                    ...
                    Ok(result) => {
                        ...
                        let _ = tx
                            .send(StreamEvent::ContentComplete {
                                stop_reason: clawreform_types::message::StopReason::EndTurn,
        ...
        let mut session = self
            ...
            .map_err(KernelError::ClawReform)?
            .unwrap_or_else(|| clawreform_memory::session::Session {
                id: entry.session_id,
                agent_id,
                messages: Vec::new(),
        ...
        let needs_compact = {
            use clawreform_runtime::compactor::{
        ...
        // Build the structured system prompt via prompt_builder
        {
            ...
            let prompt_ctx = clawreform_runtime::prompt_builder::PromptContext {
                ...
                workspace_context: manifest.workspace.as_ref().map(|w| {
                    let mut ws_ctx =
                        clawreform_runtime::workspace_context::WorkspaceContext::detect(w);
            ...
            manifest.model.system_prompt =
                clawreform_runtime::prompt_builder::build_system_prompt(&prompt_ctx);
        }

        let memory = Arc::clone(&self.memory);
        // Build link context from user message (auto-extract URLs for the agent)
        let message_owned = if let Some(link_ctx) =
            clawreform_runtime::link_understanding::build_link_context(message, &self.config.links)
        {
        ...
        let handle = tokio::spawn(async move {
            ...
            let phase_cb: clawreform_runtime::agent_loop::PhaseCallback =
                std::sync::Arc::new(move |phase| {
                    use clawreform_runtime::agent_loop::LoopPhase;
    ...
    async fn execute_wasm_agent(
        ...
        Ok(AgentLoopResult {
            response,
            total_usage: clawreform_types::message::TokenUsage {
                input_tokens: 0,
                output_tokens: 0,
            },
            iterations: 1,
    ...
    async fn execute_llm_agent(
        ...
        let is_stable = self.config.mode == clawreform_types::config::KernelMode::Stable;
        ...
        if is_stable {
            ...
            let probe = CompletionRequest {
                ...
                messages: vec![clawreform_types::message::Message::user(message)],
                tools: tools.clone(),
        ...
        match self.config.usage_footer {
            clawreform_types::config::UsageFooterMode::Off => {
                result.cost_usd = None;
            }
            clawreform_types::config::UsageFooterMode::Cost
            | clawreform_types::config::UsageFooterMode::Full => {
                result.cost_usd = if cost > 0.0 { Some(cost) } else { None };
            }
            clawreform_types::config::UsageFooterMode::Tokens => {
    ...
    pub fn switch_agent_session(
        ...
        let session = self
            ...
            .map_err(KernelError::ClawReform)?
            .ok_or_else(|| {
                KernelError::ClawReform(ClawReformError::Internal("Session not found".to_string()))
            })?;

        if session.agent_id != agent_id {
            return Err(KernelError::ClawReform(ClawReformError::Internal(
    ...
    fn save_session_summary(
        ...
        agent_id: AgentId,
        entry: &AgentEntry,
        session: &clawreform_memory::session::Session,
    ) {
        use clawreform_types::message::{MessageContent, Role};
        ...
        if !skills.is_empty() {
            ...
            for name in &skills {
                if !known.contains(name) {
                    return Err(KernelError::ClawReform(ClawReformError::Internal(format!(
                        "Unknown skill: {name}"
                    ))));
                }
            }
        ...
        if !servers.is_empty() {
            if let Ok(mcp_tools) = self.mcp_tools.lock() {
                ...
                for tool in mcp_tools.iter() {
                    if let Some(s) = clawreform_runtime::mcp::extract_mcp_server(&tool.name) {
                        known_servers.insert(s.to_string());
                    }
                }
                for name in &servers {
                    let normalized = clawreform_runtime::mcp::normalize_name(name);
        ...
        let (input_tokens, output_tokens) = session
            .map(|s| {
                ...
                for msg in &s.messages {
                    ...
                    let tokens = len / 4;
                    match msg.role {
                        clawreform_types::message::Role::User => input += tokens,
                        clawreform_types::message::Role::Assistant => output += tokens,
                        clawreform_types::message::Role::System => input += tokens,
                    }
                }
                (input, output)
            })
    ...
    pub async fn compact_agent_session(&self, agent_id: AgentId) -> KernelResult<String> {
        use clawreform_runtime::compactor::{compact_session, needs_compaction, CompactionConfig};
        ...
        let result = compact_session(driver, &model, &session, &config)
            .await
            .map_err(|e| KernelError::ClawReform(ClawReformError::Internal(e)))?;
    ...
    pub fn context_report(
        &self,
        agent_id: AgentId,
    ) -> KernelResult<clawreform_runtime::compactor::ContextReport> {
        use clawreform_runtime::compactor::generate_context_report;
        use clawreform_runtime::tool_runner::builtin_tool_definitions;
    ...
    pub fn kill_agent(&self, agent_id: AgentId) -> KernelResult<()> {
        ...
        self.audit_log.record(
            agent_id.to_string(),
            clawreform_runtime::audit::AuditAction::AgentKill,
    ...
    pub fn activate_hand(
    ...
    ) -> KernelResult<clawreform_hands::HandInstance> {
        use clawreform_hands::HandError;

        let def = self
            .hand_registry
            .get_definition(hand_id)
            .ok_or_else(|| {
                KernelError::ClawReform(ClawReformError::AgentNotFound(format!(
        ...
        let instance = self
            ...
            .map_err(|e| match e {
                HandError::AlreadyActive(id) => KernelError::ClawReform(ClawReformError::Internal(
                    format!("Hand already active: {id}"),
                )),
                other => KernelError::ClawReform(ClawReformError::Internal(other.to_string())),
            })?;
        ...
        let mut manifest = AgentManifest {
            ...
            exec_policy: if def.tools.iter().any(|t| t == "shell_exec") {
                Some(clawreform_types::config::ExecPolicy {
                    mode: clawreform_types::config::ExecSecurityMode::Full,
        ...
        let resolved = clawreform_hands::resolve_settings(&def.settings, &instance.config);
        ...
        self.hand_registry
            ...
            .map_err(|e| KernelError::ClawReform(ClawReformError::Internal(e.to_string())))?;
    ...
    pub fn list_bindings(&self) -> Vec<clawreform_types::config::AgentBinding> {
        self.bindings
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .clone()
    ...
    pub fn add_binding(&self, binding: clawreform_types::config::AgentBinding) {
    ...
    pub fn remove_binding(&self, index: usize) -> Option<clawreform_types::config::AgentBinding> {
        let mut bindings = self.bindings.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn apply_hot_actions(
        ...
        new_config: &clawreform_types::config::KernelConfig,
    ) {
    ...
    pub fn register_trigger(
        ...
        if self.registry.get(agent_id).is_none() {
            return Err(KernelError::ClawReform(ClawReformError::AgentNotFound(
                agent_id.to_string(),
            )));
        }
        Ok(self
    ...
    pub async fn run_workflow(
        ...
        let run_id = self
            ...
            .await
            .ok_or_else(|| {
                KernelError::ClawReform(ClawReformError::Internal("Workflow not found".to_string()))
            })?;
        ...
        .map_err(|e| {
            KernelError::ClawReform(ClawReformError::Internal(format!("Workflow failed: {e}")))
        })?;
        ...
        // Probe local providers for reachability and model discovery
        {
            ...
            tokio::spawn(async move {
                ...
                for (provider_id, base_url) in &local_providers {
                    let result =
                        clawreform_runtime::provider_health::probe_provider(provider_id, base_url)
                            .await;
                    if result.reachable {
                        info!(
    ...
    async fn start_ofp_node(self: &Arc<Self>) {
        use clawreform_wire::{PeerConfig, PeerNode, PeerRegistry};
        ...
        let node_name = gethostname().unwrap_or_else(|| "clawreform-node".to_string());
        ...
        let handle: Arc<dyn clawreform_wire::peer::PeerHandle> = self.self_arc();
        ...
        match PeerNode::start(peer_config, registry.clone(), handle.clone()).await {
            Ok((node, _accept_task)) => {
                ...
                let self_ptr = Arc::as_ptr(self) as *mut ClawReformKernel;
                unsafe {
    ...
    pub fn shutdown(&self) {
        info!("Shutting down ClawReform kernel...");
        ...
        info!(
            "ClawReform kernel shut down ({} agents preserved)",
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        if !manifest.fallback_models.is_empty() {
            ...
            if chain.len() > 1 {
                return Ok(Arc::new(
                    clawreform_runtime::drivers::fallback::FallbackDriver::new(chain),
                ));
            }
        }
    ...
    async fn connect_mcp_servers(self: &Arc<Self>) {
        use clawreform_runtime::mcp::{McpConnection, McpServerConfig, McpTransport};
        use clawreform_types::config::McpTransportEntry;
        ...
        if let Ok(mcp_tools) = self.mcp_tools.lock() {
            if mcp_allowlist.is_empty() {
                ...
                let normalized: Vec<String> = mcp_allowlist
                    .iter()
                    .map(|s| clawreform_runtime::mcp::normalize_name(s))
                    .collect();
                all_tools.extend(
                    mcp_tools
                        .iter()
                        .filter(|t| {
                            clawreform_runtime::mcp::extract_mcp_server(&t.name)
    ...
    pub fn reload_skills(&self) {
        ...
        let mut fresh = clawreform_skills::registry::SkillRegistry::new(skills_dir);
                ...
                if let Some(ref ctx) = skill.manifest.prompt_context {
                    if !ctx.is_empty() {
                        let is_bundled = matches!(
                            ...
                            Some(clawreform_skills::SkillSource::Bundled)
                        );
                        if is_bundled {
...
async fn cron_deliver_response(
    kernel: &ClawReformKernel,
    agent_id: AgentId,
    response: &str,
    delivery: &clawreform_types::scheduler::CronDelivery,
) {
    use clawreform_types::scheduler::CronDelivery;
...
#[async_trait]
impl KernelHandle for ClawReformKernel {
    async fn spawn_agent(
        ...
        let content_hash = clawreform_types::manifest_signing::hash_manifest(manifest_toml);
    ...
    fn kill_agent(&self, agent_id: &str) -> Result<(), String> {
        ...
        ClawReformKernel::kill_agent(self, id).map_err(|e| format!("Kill failed: {e}"))
    }
    ...
    async fn publish_event(
        ...
        ClawReformKernel::publish_event(self, event).await;
        Ok(())
    }

    async fn knowledge_add_entity(
        &self,
        entity: clawreform_types::memory::Entity,
    ) -> Result<String, String> {
        self.memory
            .add_entity(entity)
            .await
    ...
    async fn knowledge_add_relation(
        &self,
        relation: clawreform_types::memory::Relation,
    ) -> Result<String, String> {
        self.memory
    ...
    async fn knowledge_query(
        &self,
        pattern: clawreform_types::memory::GraphPattern,
    ) -> Result<Vec<clawreform_types::memory::GraphMatch>, String> {
        self.memory
            .query_graph(pattern)
            .await
    ...
    async fn cron_create(
        ...
        use clawreform_types::scheduler::{
        ...
        let aid = clawreform_types::agent::AgentId(
    ...
    async fn cron_cancel(&self, job_id: &str) -> Result<(), String> {
        let id = clawreform_types::scheduler::CronJobId(
    ...
    async fn request_approval(
        ...
        use clawreform_types::approval::{ApprovalDecision, ApprovalRequest as TypedRequest};
    ...
    async fn send_channel_message(
        ...
        let user = clawreform_channels::types::ChannelUser {
            platform_id: recipient.to_string(),
            display_name: recipient.to_string(),
            clawreform_user: None,
        };
    ...
    async fn spawn_agent_checked(
        ...
        manifest_toml: &str,
        parent_id: Option<&str>,
        parent_caps: &[clawreform_types::capability::Capability],
    ) -> Result<(String, String), String> {
        ...
        clawreform_types::capability::validate_capability_inheritance(parent_caps, &child_caps)?;
...
#[async_trait]
impl clawreform_wire::peer::PeerHandle for ClawReformKernel {
    fn local_agents(&self) -> Vec<clawreform_wire::message::RemoteAgentInfo> {
        self.registry
            .list()
            .iter()
            .map(|entry| clawreform_wire::message::RemoteAgentInfo {
    ...
    fn discover_agents(&self, query: &str) -> Vec<clawreform_wire::message::RemoteAgentInfo> {
        let q = query.to_lowercase();
        self.registry
            .list()
            .iter()
...
mod tests {
    ...
    fn test_manifest_to_capabilities_with_profile() {
        use clawreform_types::agent::ToolProfile;
        let manifest = AgentManifest {
