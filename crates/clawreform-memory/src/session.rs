...
impl SessionStore {
    ...
    pub fn get_session(&self, session_id: SessionId) -> ClawReformResult<Option<Session>> {
        let conn = self
            .conn
            .lock()
        ...
        match result {
            ...
            Err(e) => Err(ClawReformError::Memory(e.to_string())),
        }
    }

    /// Save a session to the database.
    pub fn save_session(&self, session: &Session) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
    ...
    pub fn delete_session(&self, session_id: SessionId) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
    ...
    pub fn delete_agent_sessions(&self, agent_id: AgentId) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
    ...
    pub fn list_sessions(&self) -> ClawReformResult<Vec<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
        ...
        for row in rows {
            sessions.push(row.map_err(|e| ClawReformError::Memory(e.to_string()))?);
        }
        Ok(sessions)
    }

    /// Create a new empty session for an agent.
    pub fn create_session(&self, agent_id: AgentId) -> ClawReformResult<Session> {
        let session = Session {
            id: SessionId::new(),
            agent_id,
            messages: Vec::new(),
    ...
    pub fn find_session_by_label(
        ...
        agent_id: AgentId,
        label: &str,
    ) -> ClawReformResult<Option<Session>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
...
impl SessionStore {
    ...
    pub fn load_canonical(&self, agent_id: AgentId) -> ClawReformResult<CanonicalSession> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
    ...
    ) -> ClawReformResult<CanonicalSession> {
        let mut canonical = self.load_canonical(agent_id)?;
            ...
            if to_compact > canonical.compaction_cursor {
                ...
                for msg in compacting {
                    let role = match msg.role {
                        clawreform_types::message::Role::User => "User",
                        clawreform_types::message::Role::Assistant => "Assistant",
                        clawreform_types::message::Role::System => "System",
                    };
    ...
    ) -> ClawReformResult<(Option<String>, Vec<Message>)> {
    ...
    fn save_canonical(&self, canonical: &CanonicalSession) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
...
mod tests {
    ...
    fn test_jsonl_mirror_write() {
        ...
        session
            .messages
            .push(clawreform_types::message::Message::user("Hello"));
        session
            .messages
            .push(clawreform_types::message::Message::assistant("Hi there!"));
        store.save_session(&session).unwrap();
