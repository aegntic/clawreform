...
use chrono::Utc;
use clawreform_types::agent::{AgentEntry, AgentId};
...
impl StructuredStore {
    ...
    pub fn get(&self, agent_id: AgentId, key: &str) -> ClawReformResult<Option<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn delete(&self, agent_id: AgentId, key: &str) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
    ...
    pub fn list_kv(&self, agent_id: AgentId) -> ClawReformResult<Vec<(String, serde_json::Value)>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
        ...
        for row in rows {
            let (key, val_str) = row.map_err(|e| ClawReformError::Memory(e.to_string()))?;
    ...
    pub fn save_agent(&self, entry: &AgentEntry) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
    ...
    pub fn load_agent(&self, agent_id: AgentId) -> ClawReformResult<Option<AgentEntry>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
        ...
        match result {
            Ok((name, manifest_blob, state_str, created_str, session_id_str)) => {
                ...
                let session_id = session_id_str
                    ...
                    .map(clawreform_types::agent::SessionId)
                    .unwrap_or_else(clawreform_types::agent::SessionId::new);
                Ok(Some(AgentEntry {
                    id: agent_id,
                    name,
                    manifest,
    ...
    pub fn remove_agent(&self, agent_id: AgentId) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
    ...
    pub fn load_all_agents(&self) -> ClawReformResult<Vec<AgentEntry>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
        ...
        for row in rows {
            ...
            let manifest: clawreform_types::agent::AgentManifest = match rmp_serde::from_slice(
                &manifest_blob,
            ) {
                Ok(m) => m,
                Err(e) => {
    ...
    pub fn list_agents(&self) -> ClawReformResult<Vec<(String, String, String)>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
        ...
        let mut agents = Vec::new();
        for row in rows {
            agents.push(row.map_err(|e| ClawReformError::Memory(e.to_string()))?);
        }
        Ok(agents)
    }
}
