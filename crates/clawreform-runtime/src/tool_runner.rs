...
use clawreform_skills::registry::SkillRegistry;
use clawreform_types::taint::{TaintLabel, TaintSink, TaintedValue};
use clawreform_types::tool::{ToolDefinition, ToolResult};
...
pub async fn execute_tool(
    ...
    exec_policy: Option<&clawreform_types::config::ExecPolicy>,
    tts_engine: Option<&crate::tts::TtsEngine>,
    docker_config: Option<&clawreform_types::config::DockerSandboxConfig>,
    ...
    let result = match tool_name {
        ...
        "shell_exec" => {
            ...
            let is_full_exec = exec_policy
                .is_some_and(|p| p.mode == clawreform_types::config::ExecSecurityMode::Full);
            if !is_full_exec {
        ...
        other => {
            ...
            // Fallback 2: Skill registry tool providers
                ...
                if let Some(skill) = registry.find_tool_provider(other) {
                    ...
                    match clawreform_skills::loader::execute_skill_tool(
                        &skill.manifest,
                        &skill.path,
                        other,
                        input,
    ...
    vec![
        ...
        ToolDefinition {
            ...
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Base URL of the remote ClawReform/A2A-compatible agent (e.g., 'https://agent.example.com')" }
                },
                "required": ["url"]
            }),
        },
...
fn parse_entity_type(s: &str) -> clawreform_types::memory::EntityType {
    use clawreform_types::memory::EntityType;
...
fn parse_relation_type(s: &str) -> clawreform_types::memory::RelationType {
    use clawreform_types::memory::RelationType;
    match s.to_lowercase().as_str() {
...
async fn tool_knowledge_add_entity(
    ...
    let entity = clawreform_types::memory::Entity {
...
async fn tool_knowledge_add_relation(
    ...
    let relation = clawreform_types::memory::Relation {
...
async fn tool_knowledge_query(
    ...
    let pattern = clawreform_types::memory::GraphPattern {
        source,
        relation,
        target,
        max_depth,
...
async fn tool_media_describe(
    ...
    let attachment = clawreform_types::media::MediaAttachment {
        media_type: clawreform_types::media::MediaType::Image,
        mime_type: mime.to_string(),
        source: clawreform_types::media::MediaSource::Base64 {
...
async fn tool_image_generate(
    ...
    let model = match model_str {
        "dall-e-3" | "dalle3" | "dalle-3" => clawreform_types::media::ImageGenModel::DallE3,
        "dall-e-2" | "dalle2" | "dalle-2" => clawreform_types::media::ImageGenModel::DallE2,
        "gpt-image-1" | "gpt_image_1" => clawreform_types::media::ImageGenModel::GptImage1,
        _ => {
            return Err(format!(
    ...
    let request = clawreform_types::media::ImageGenRequest {
...
async fn tool_speech_to_text(
    ...
    use clawreform_types::media::{MediaAttachment, MediaSource, MediaType};
...
mod tests {
    ...
    async fn test_canvas_present_tool() {
        ...
        let tmp = std::env::temp_dir().join("clawreform_canvas_test");
