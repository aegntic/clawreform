...
4. **Server thread** -- A dedicated OS thread named `"clawreform-server"` is spawned. It creates its own `tokio::runtime::Builder::new_multi_thread()` runtime and runs:
   ...
   - `run_embedded_server()` -- builds the axum router via `clawreform_api::server::build_router()`, converts the `std::net::TcpListener` to a `tokio::net::TcpListener`, and serves with graceful shutdown.
...
Opens a native file picker for `.toml` files. Validates the selected file as an `AgentManifest`, copies it to `~/.clawreform/agents/{name}/agent.toml`, and spawns the agent. Returns the agent name on success.

### `import_skill_file`

Opens a native file picker for skill files (`.md`, `.toml`, `.py`, `.js`, `.wasm`). Copies the file to `~/.clawreform/skills/` and triggers a hot-reload of the skill registry.
...
Opens `~/.clawreform/` or `~/.clawreform/logs/` in the OS file manager.
